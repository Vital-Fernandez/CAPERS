...
issues_objects_file = '/home/vital/Dropbox/Astrophysics/Data/CAPERS/CAPERS_UDS_v1_notes.ods'
issues_df = pd.read_excel(issues_objects_file, engine="odf")
nMPT_list = issues_df['Spec ID'].to_numpy()


idcs_manual = pd.notnull(files_sample.frame.z_manual) & files_sample.frame.ext.isin(['optext', 's1d'])
files_sample.frame.loc[idcs_manual]
manual_selection = files_sample.frame.loc[idcs_manual].index

idcs_bands = files_sample.frame.loc[pd.notnull(files_sample.frame.z_bands)].index
files_sample.frame.loc[idcs_bands]
bands_selection = files_sample.frame.loc[idcs_bands].index

idcs_no_lines = manual_selection.difference(bands_selection)
final_df = files_sample.frame.loc[idcs_no_lines]

final_df.sort_values('z_manual', ascending=False, inplace=True)
final_df = final_df[['MPT_number', 'z_manual']]
lime.save_frame(data_folder/'tables'/f'{sample}_non_lines_objects.txt', final_df)
# # Crop the sample to certain objects and observations types
# idcs_selection = files_sample.index.get_level_values('id').isin(nMPT_list)  # files_sample.index
...
#                             obj_redshift_check_file = bands_folder / f'{fits_stem}_redshift_review.png'
...
#                                                         output_file_log=log_address, output_address=obj_redshift_check_file,
#                                                         fig_cfg={"figure.dpi" : 400})
# #
# #                         if BANDS_CHECK:
# #
