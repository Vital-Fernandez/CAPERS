            ...
            if counter >= start_object:
                ...
                if BANDS_CHECK:
                    ...
                    if pd.notnull(z_obj):
                        ...
                        composite_lines = capers_cfg['lines_data']['line_composites_high_z']

                        # Check the bands
                        spec.check.bands(obj_bands_file, ref_bands=prism_bands_df, band_vsigma=600,
...
#                         spec.infer.components(show_steps=False, exclude_continuum=True)
#                         spec.plot.spectrum(show_categories=False, rest_frame=True, maximize=True)
#
...
#                         spec = spec if spec is not None else files_sample.get_spectrum(idx_obj, redshift=z_obj, file_path=file_path)
#
#                         # Compute the redshifts
#                         limits = (np.maximum(z_phot-2,0), np.minimum(z_phot+2, 16)) if z_phot < 3 else (None, None)
...
#
#                         if z_tier < z_tier_aspect:
#                             files_sample.loc[idx_obj, 'z_tier'] = z_tier
#
#                         # Output message
#                         print(f'- Aspect Check: z_phot = {z_phot}')
#                         print(f'-- Key: z_key = {z_key}')
...
#                 # Review the bands
#                 if BANDS_CHECK:
#                     if pd.notnull(z_obj) and (z_tier > 1):
#
#                         # Load the object
#                         print(f'- Bands check: z_bands = {z_obj}')
#                         spec = spec if spec is not None else files_sample.get_spectrum(idx_obj, redshift=z_obj, file_path=file_path)
...
#                                          obj_cfg_prefix=obj_cfg_section, band_vsigma=600,
#                                          vacuum_waves=True, components_detection=True, exclude_continua=True)
#
#                         # Save the z_bands to the dataframe
#                         if pd.isnull(files_sample.loc[idx_obj, 'z_bands']) and obj_bands_file.is_file():
#                             files_sample.loc[idx_obj, 'z_bands'] = z_obj
...
#                         else:
#                             input_bands = spec.retrieve.line_bands(ref_bands=prism_bands_df, fit_conf=capers_cfg,
#                                                                    # default_conf_prefix='automatic_prism',
#                                                                    default_conf_prefix='default_prism',
#                                                                    obj_conf_prefix=obj_cfg_section, band_vsigma=600,
...
#                                            default_conf_prefix='default_prism', cont_from_bands=False, err_from_bands=False)
#                             safety_check = True
#
#                             # spec.plot.spectrum(bands=input_bands, show_categories=False, rest_frame=True)
#
#                         except:
#                             print('- z_lime fitting: Failed')
#                             safety_check = True
#                             failing_objects[fits_stem] = MPT
#
...
#                         except:
#                             failing_objects[f'{fits_stem}_grid'] = MPT
#
